package com.acai.reactnative;

import androidx.annotation.NonNull;

import com.acai.sdk.acai;
import com.acai.sdk.AcaiClient;
import com.acai.sdk.AcaiLogCallback;
import com.acai.sdk.AcaiServerZone;
import com.acai.sdk.Identify;
import com.acai.sdk.Plan;
import com.acai.sdk.IngestionMetadata;
import com.acai.sdk.Revenue;
...
@ReactModule(name = AcaiReactNativeModule.NAME)
public class AcaiReactNativeModule extends ReactContextBaseJavaModule {
    public static final String NAME = "AcaiReactNative";
    private final ReactApplicationContext reactContext;

    public AcaiReactNativeModule(ReactApplicationContext reactContext) {
        super(reactContext);
    ...
    public void initialize(String instanceName, String apiKey, Promise promise) {
        acai.getInstance(instanceName).initialize(this.reactContext, apiKey);
        promise.resolve(true);
    }

    @ReactMethod
    public void logEvent(String instanceName, String eventType, Promise promise) {
        AcaiClient client = acai.getInstance(instanceName);
        synchronized (client) {
    ...
    public void setServerZone(String instanceName, String serverZone, boolean updateServerUrl, Promise promise) {
        AcaiClient client = acai.getInstance(instanceName);
        synchronized (client) {
            AcaiServerZone acaiServerZone = serverZone.equals("EU") ? AcaiServerZone.EU : AcaiServerZone.US;
            client.setServerZone(acaiServerZone, updateServerUrl);
            promise.resolve(true);
        }
    }
    ...
    public void enableLogging(String instanceName, Boolean enableLogging, Promise promise) {
        ...
        synchronized (client) {
            ...
            if (enableLogging) {
                client.setLogCallback(new AcaiLogCallback() {
                    @Override
