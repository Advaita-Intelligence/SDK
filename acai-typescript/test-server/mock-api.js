...
export function configureMockApiMiddleware(middlewares) {
  ...
  middlewares.use((req, res, next) => {
    ...
    if (statusMatch) {
      ...
      res.setHeader('Authorization', 'SECRET TOKEN, THIS SHOULD NEVER BE CAPTURED BY ACAI');
 ...
 * Example: https://cdn.acai.yourdomain.com/libs/analytics-browser-2.33.5-min.js.gz:1:54975
 * Returns: { packageName: 'analytics-browser', version: '2.33.5', suffix: 'min', line: 1, column: 54975 }
 */
function parseCdnUrl(stacktraceLine) {
  // Pattern: https://cdn.acai.yourdomain.com/libs/{name}-{version}-{suffix}.js.gz:{line}:{column}
  // The suffix could be 'min' or 'gtm-min' etc.
  const regex = /https?:\/\/cdn\.acai\.com\/libs\/([a-z-]+)-([\d.]+)-([a-z-]+)\.js(?:\.gz|\.br)?:(\d+):(\d+)/i;
 ...
 * Example: analytics-browser-2.33.5-min.js.gz → https://cdn.acai.yourdomain.com/libs/analytics-browser-2.33.5-min.js.map
 */
function getSourceMapCdnUrl(parsedUrl) {
  const { packageName, version, suffix } = parsedUrl;
  return `https://cdn.acai.yourdomain.com/libs/${packageName}-${version}-${suffix}.js.map`;
}
...
export function configureUnminifyMiddleware(middlewares) {
  middlewares.use('/api/unminify', async (req, res) => {
    ...
    if (!parsed) {
      ...
      res.end(JSON.stringify({ 
        success: false, 
        error: 'Could not parse stack trace line. Expected format: https://cdn.acai.yourdomain.com/libs/{package}-{version}-{suffix}.js.gz:{line}:{column}' 
      }));
      return;
    }
    ...
    try {
      ...
      // The tag is always @acai/{cdnPackageName}@{version} (e.g., @acai/analytics-browser@2.33.5)
      let githubLink = null;
      if (sourceInfo.relativePath && !sourceInfo.isExternal) {
        const tag = `@acai/${parsed.packageName}@${parsed.version}`;
        githubLink = `https://github.com/YOUR_USERNAME/Acai-TypeScript/blob/${encodeURIComponent(tag)}/${sourceInfo.relativePath}#L${original.line}`;
      }
        ...
        res.end(JSON.stringify({ 
          ...
          instructions: {
            ...
            steps: [
              ...
              `2. Find the source map: packages/${parsed.packageName}/lib/scripts/acai-${parsed.suffix}.js.map`,
