import { createInstance } from '@acai/analytics-browser';
import { EnrichmentPlugin } from '@acai/analytics-types';
import { BaseEvent } from '@acai/analytics-types/src';
 ...
 * values set by the @acai/analytics-browser library, for example:
 * - `event.time`
 * - `event.idfa`
 * - `event.idva`
 * - `event.ip`
 ...
 * is set to 1000 milliseconds. In rare cases where initial request to Acai fails due to
