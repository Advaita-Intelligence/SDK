 ...
 * more event_properties on top of what @acai/analytics-browser provides out of the box
 *
...
export const pageViewTrackingEnrichment = (): EnrichmentPlugin => {
  return {
    ...
    execute: async (event) => {
      if (event.event_type !== '[Acai] Page Viewed') {  // event name format if using Autocapture Pageviews 
        return event;
      }
