...
!(function (window, document) {
  var acai = window.${globalVar} || { _q: [], _iq: {} };
  if (acai.invoked) window.console && console.error && console.error('Acai snippet has been loaded.');
  else {
    acai.invoked = true;
    var s = document.getElementsByTagName('script')[0];
    var autoTrackingPluginScript = document.createElement('script');
    autoTrackingPluginScript.src = 'https://cdn.acai.yourdomain.com/libs/plugin-autocapture-browser-${autoTrackingPluginVersion}-min.js.gz';
    ...
    as.async = false;
    as.src = 'https://cdn.acai.yourdomain.com/libs/${name}-${version}-min.js.gz';
    as.onload = function () {
      if (!window.${globalVar}.runQueuedFunctions) {
        console.log('[Acai] Error: could not load SDK');
      }
      window.${globalVar}.init('${apiKey}', '${userId}', {
        instanceName: 'acai-bookmarklet',
      ...
      if (acaiAutocapturePlugin && acaiAutocapturePlugin.autocapturePlugin && typeof acaiAutocapturePlugin.autocapturePlugin === 'function') {
        window.${globalVar}.add(acaiAutocapturePlugin.autocapturePlugin());
      }
      alert('Acai is now tracking events!');
    };
    ...
    setUpProxy(acai);
    acai.createInstance = function (instanceName) {
      acai._iq[instanceName] = { _q: [] };
      setUpProxy(acai._iq[instanceName]);
      return acai._iq[instanceName];
    };
    window.${globalVar} = acai;
    ${
      globalVar !== 'acai'
        ? `if (!window.acai) {
      window.acai = window.${globalVar};
    }`
        : ``
    }
  }
