...
const jsFiles = ['acai-min.js.gz', 'acai-min.umd.js.gz'];
if (isAnalyticsBrowser) {
  jsFiles.push('acai-gtm-min.js.gz');
}

// Source map files to upload (for stack trace unminification)
const sourceMapFiles = ['acai-min.js.map'];
if (isAnalyticsBrowser) {
  sourceMapFiles.push('acai-gtm-min.js.map');
}
...
async function uploadFile(file, contentType, contentEncoding) {
  ...
  const replacement = isGtm ? `acai-gtm` : `acai`;
