...
DEPRECATED_PACKAGES=("@acai/analytics-types" "@acai/analytics-client-common" "@amplitude/analytics-remote-config")
  ...
  for PACKAGE in "${DEPRECATED_PACKAGES[@]}"; do
    ...
    if [ "$CURRENT_HAS" -gt 0 ] && [ "$BASE_HAS" -eq 0 ]; then
      ...
      if [ "$PACKAGE" = "@amplitude/analytics-remote-config" ]; then
        echo "   Please use the new remote config client in @acai/analytics-core instead"
      else
        echo "   Please use @acai/analytics-core instead"
      fi
      FAILED=1
...
if [ "$FAILED" -eq 1 ]; then
  ...
  echo ""
  echo "For @acai/analytics-types and @acai/analytics-client-common:"
  echo "  → Use @acai/analytics-core instead"
  echo ""
  echo "For @amplitude/analytics-remote-config:"
  echo "  → Use the new remote config client in @acai/analytics-core instead"
  echo ""
