...
class Acai {
  ...
  MethodChannel _channel = const MethodChannel('acai_flutter');

  /// Whether the Amplitude instance has been successfully initialized
  ///
  /// ```
  /// var amplitude = Acai(Configuration(apiKey: 'apiKey'));
  ...
  ///
  /// ```
  /// final amplitude = Acai(Configuration(apiKey: 'apiKey'));
  /// // If care about init complete
  /// await amplitude.isBuilt;
  /// ```
  Acai(this.configuration, [MethodChannel? methodChannel]) {
  ...
  Future<bool> _init() async {
    try {
      ...
      print('Error initializing Acai: $e');
