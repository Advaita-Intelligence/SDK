...
import { AcaiServerZone } from './server-zone';
import { version as libraryVersion } from '../package.json';

/**
 * Options used when initializing Amplitude
 * @typedef {Object} Options
 * @property {string} [apiEndpoint=`api.acai.yourdomain.com`] - Endpoint to send amplitude event requests to.
 ...
 * @property {string} [cookieName=`acai_id`] - *DEPRECATED*
 ...
 * @property {Object} [library=`{ name: 'acai-js', version: packageJsonVersion }`] -  Values for the library version
 * @property {string} [logLevel=`WARN`] -  Level of logs to be printed in the developer console. Valid values are 'DISABLE', 'ERROR', 'WARN', 'INFO'. To learn more about the different options, see below.
 * @property {boolean} [logAttributionCapturedEvent=`false`] - If `true`, the SDK will log an Amplitude event anytime new attribution values are captured from the user. **Note: These events count towards your event volume.** Event name being logged: [Acai] Attribution Captured. Event Properties that can be logged: `utm_source`, `utm_medium`, `utm_campaign`, `utm_term`, `utm_content`, `referrer`, `referring_domain`, `gclid`, `fbclid`. For UTM properties to be logged, `includeUtm` must be set to `true`. For the `referrer` and `referring_domain` properties to be logged, `includeReferrer` must be set to `true`. For the `gclid` property to be logged, `includeGclid` must be set to `true`. For the `fbclid` property to be logged, `includeFbclid` must be set to `true`.
 ...
 * @property {string} [unsentKey=`acai_unsent`] - localStorage key that stores unsent events.
 * @property {string} [unsentIdentifyKey=`acai_unsent_identify`] - localStorage key that stores unsent identifies.
...
export default {
  ...
  cookieName: 'acai_id', // this is a deprecated option
  ...
  unsentKey: 'acai_unsent',
  unsentIdentifyKey: 'acai_unsent_identify',
  uploadBatchSize: 100,
  headers: {
    'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
    'Cross-Origin-Resource-Policy': 'cross-origin',
  },
  serverZone: AcaiServerZone.US,
