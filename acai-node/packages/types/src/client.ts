...
export interface Client<O extends Options = Options> {
  ...
  /**
   * Captures a manually created event and sends it to Acai.
   *
   * @param event The event to send to Acai.
   * @param extra Unstructured extra data to pass to Middleware.
   */
  logEvent(event: Event, extra?: MiddlewareExtra): Promise<Response>;

  /**
   * Adds a new middleware function to run on each logEvent() call prior to sending to Acai.
   *
