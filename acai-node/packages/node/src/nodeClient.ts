...
import { logger, isNodeEnv, isValidEvent } from '@acai/utils';
...
export class NodeClient implements Client<Options> {
  ...
  public constructor(apiKey: string, options: Partial<Options> = {}) {
    ...
    if (!isNodeEnv()) {
      logger.warn(
        '@acai/node initialized in a non-node environment and will not work. If you are planning to add Acai to a browser environment, please use a browser-compatible SDK',
      );
    }
  }
   ...
   * @returns a Promise containing metadata about the success of sending this identify to the Acai API
   */
