...
from acai.plugin import AcaiDestinationPlugin, ContextPlugin, EventPlugin, DestinationPlugin
from acai import Acai, PluginType, BaseEvent, RevenueEvent, IdentifyEvent, GroupIdentifyEvent, Config, Plan


class AcaiPluginTestCase(unittest.TestCase):

    def test_plugin_initialize_amplitude_client_setup_destination_plugin_success(self):
        destination_setup = MagicMock()
        AcaiDestinationPlugin.setup = destination_setup
        client = Acai("test_api_key")
        timeline = client._Acai__timeline
    ...
    def test_plugin_context_plugin_execute_event_success(self):
        ...
        context_plugin.setup(Acai("test_api_key"))
