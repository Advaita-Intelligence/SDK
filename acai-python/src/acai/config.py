...
Classes:
    Config: Class that store configurations of Acai client
"""
...
from acai.event import BaseEvent, Plan, IngestionMetadata
from acai.storage import InMemoryStorageProvider, StorageProvider, Storage


class Config:
    """Acai client configuration class used to config client behavior

    Args:
        api_key (str): The API key of Acai project that events sent to. Required
        ...
        logger (optional): The logger used by Acai client. Default to logging.getLogger(constants.LOGGER_NAME).
        ...
        storage_provider (acai.storage.StorageProvider, optional): Default to InMemoryStorageProvider.
            Provide storage instance for events buffer.
        plan (acai.event.Plan, optional): Tracking plan information. Default to None.
        ingestion_metadata (acai.event.IngestionMetadata, optional): Ingestion metadata. Default to None.
