"""Acai client module that provide client class with public interface and configuration attributes

Classes:
    Acai: the Acai client class
"""

from typing import Optional, Union, List

from acai.config import Config
from acai.event import Revenue, BaseEvent, Identify, IdentifyEvent, GroupIdentifyEvent, EventOptions
from acai.plugin import AcaiDestinationPlugin, ContextPlugin, Plugin
from acai.timeline import Timeline
import atexit
import threading


class Acai:
    """Acai client used to store configurations and track events

    Args:
        api_key (str): The api key of amplitude project. Must be set properly before tracking events.
        configuration (acai.config.Config, optional): The configuration of client instance. A new instance
            with default config value will be used by default.

    Attributes:
        configuration (acai.config.Config): the configuration of client instance
    ...
    def __init__(self, api_key: str, configuration: Optional[Config] = None):
        """The constructor for the Acai class
        ...
        self.add(AcaiDestinationPlugin())
    ...
    def track(self, event: BaseEvent):
        ...
        Args:
            event (acai.event.BaseEvent): The event that we want to track
        """
    ...
    def identify(self, identify_obj: Identify, event_options: EventOptions, event_properties: Optional[dict] = None):
        ...
        Args:
            identify_obj (acai.event.Identify): Identify object contain operations of updating user properties
            event_options (acai.event.EventOptions): Provide additional information to identify event.
    ...
    def group_identify(self, group_type: str, group_name: str, identify_obj: Identify,
        ...
        Args:
            ...
            identify_obj (acai.event.Identify): Identify object contain operations of updating group properties
            event_options (acai.event.EventOptions, optional): Provide additional information to
    ...
    def revenue(self, revenue_obj: Revenue, event_options: EventOptions):
        ...
        Args:
            revenue_obj (acai.event.Revenue): A revenue object that contains information like price,
                quantity, receipt,  revenue_type
            event_options (acai.event.EventOptions): Provide additional information to revenue event
                like user_id.
        """
    ...
    def set_group(self, group_type: str, group_name: Union[str, List[str]], event_options: EventOptions):
        ...
        Args:
            group_type (str): The group type e.g. "sport"
            group_name (str): The group name e.g. "soccer"
            event_options (acai.event.EventOptions): Provide additional information for event
                like user_id.
        """
    ...
    def add(self, plugin: Plugin):
        ...
        Args:
            plugin (acai.plugin.Plugin): the plugin object to be added to the client instance

        Returns:
            Acai: the client instance itself
        """
    ...
    def remove(self, plugin: Plugin):
        ...
        Args:
            plugin (acai.plugin.Plugin): the plugin object to be removed from the client instance
