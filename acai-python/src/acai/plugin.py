...
Classes:
    ...
    AcaiDestinationPlugin: Default Acai Destination plugin that send events to Acai.
    ContextPlugin: A default plugin that add library info to event. Also set event default timestamp and insert_id
        if not set elsewhere.

Methods:
    verify_event(event): Perform basic validation before AcaiDestinationPlugin send the event to storage.
"""
...
from acai.event import BaseEvent, GroupIdentifyEvent, IdentifyEvent, RevenueEvent
from acai import constants
from acai.timeline import Timeline
from acai.exception import InvalidEventError
from acai import utils
...
class EventPlugin(Plugin):
    ...
    Methods:
        setup(client): Setup plugin using Acai client instance.
        execute(event): Method to override to process event. Return modified event or None. Return None will stop the
            Acai client sending the event and callback will not be triggered.
...
class DestinationPlugin(EventPlugin):
    """Plugins that send events to a destination like Acai.
...
class AcaiDestinationPlugin(DestinationPlugin):
    """The Acai destination plugin. Added to client by default. Send events to Acai.
    ...
    def __init__(self):
        """The constructor of AcaiDestinationPlugin class"""
        super().__init__()
    ...
    def setup(self, client):
        ...
        Args:
            client: The Acai client that holds the destination plugin.
        """
        super().setup(client)
...
class ContextPlugin(Plugin):
    """Acai Context plugin. Default added to client. Add library info to event.
...
def verify_event(event):
    """Perform basic validation before AcaiDestinationPlugin send the event to storage.
