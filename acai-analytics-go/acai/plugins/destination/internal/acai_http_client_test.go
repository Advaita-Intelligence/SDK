...
func TestAcaiHTTPClient(t *testing.T) {
	suite.Run(t, new(AcaiHTTPClientSuiteSuite))
}

type AcaiHTTPClientSuiteSuite struct {
	suite.Suite
}

func (t *AcaiHTTPClientSuiteSuite) TestSend_Success() {
	server := t.createTestServer(0, 200, `{"code": 234, "error": "some server error"}`)

	client := internal.NewAcaiHTTPClient(
		server.URL,
		internal.AcaiPayloadOptions{MinIDLength: 7},
		noopLogger{},
		time.Millisecond*1000,
	)

	response := client.Send(internal.AcaiPayload{
		APIKey: "my-api-key",
		Events: []*types.Event{
			t.createEvent(3),
			t.createEvent(1),
	...
	t.Require().Equal(internal.AcaiResponse{
		Status: 200,
		Code:   234,
...
func (t *AcaiHTTPClientSuiteSuite) TestSend_Empty() {
	server := t.createTestServer(0, 200, `{"code": 234, "error": "some server error"}`)
	...
	t.Require().Equal(internal.AcaiResponse{}, response)
...
func (t *AcaiHTTPClientSuiteSuite) TestSend_Timeout() {
...
func (t *AcaiHTTPClientSuiteSuite) TestSend_NonJsonResponse() {
...
func (t *AcaiHTTPClientSuiteSuite) createTestServer(delay time.Duration, statusCode int, responseBody string) *testServer {
	server := &testServer{}
...
func (t *AcaiHTTPClientSuiteSuite) createEvent(index int) *types.Event {
