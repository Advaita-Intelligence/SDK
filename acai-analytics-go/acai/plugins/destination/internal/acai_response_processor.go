...
type AcaiResponseProcessor interface {
	Process(events []*types.StorageEvent, response AcaiResponse) AcaiProcessorResult
}

type AcaiProcessorResult struct {
	Code              int
	Message           string
...
func NewAcaiResponseProcessor(options AcaiResponseProcessorOptions) AcaiResponseProcessor {
	return &acaiResponseProcessor{
		Options: options,
	}
}

type AcaiResponseProcessorOptions struct {
	MaxRetries             int
...
type acaiResponseProcessor struct {
	Options AcaiResponseProcessorOptions
}

func (p *acaiResponseProcessor) Process(events []*types.StorageEvent, response AcaiResponse) AcaiProcessorResult {
	...
	var result AcaiProcessorResult
...
func (p *acaiResponseProcessor) processSuccess(events []*types.StorageEvent, response AcaiResponse) AcaiProcessorResult {
	return AcaiProcessorResult{
		Code:              response.Code,
...
func (p *acaiResponseProcessor) processTimeout(events []*types.StorageEvent, response AcaiResponse) AcaiProcessorResult {
...
func (p *acaiResponseProcessor) processTooLargeRequest(events []*types.StorageEvent, response AcaiResponse) AcaiProcessorResult {
	if len(events) == 1 {
		result := AcaiProcessorResult{
			Code:              response.Code,
...
func (p *acaiResponseProcessor) processBadRequest(events []*types.StorageEvent, response AcaiResponse) AcaiProcessorResult {
	switch {
...
func (p *acaiResponseProcessor) processTooManyRequests(events []*types.StorageEvent, response AcaiResponse) AcaiProcessorResult {
	eventsForCallback := make([]*types.StorageEvent, 0, len(events))
	eventsForRetry := make([]*types.StorageEvent, 0, len(events))
...
func (p *acaiResponseProcessor) processUnknownError(events []*types.StorageEvent, response AcaiResponse) AcaiProcessorResult {
...
func (p *acaiResponseProcessor) retryInterval(retries int) time.Duration {
