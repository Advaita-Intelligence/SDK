...
class UIKitElementInteractions {
    ...
    fileprivate static let amplitudeInstances = NSHashTable<Acai>.weakObjects()
    ...
    static func register(_ amplitude: Acai) {
        let manager = amplitude.autocaptureManager

        lock.withLock {
            acaiInstances.add(amplitude)
    ...
    static func unregister(_ amplitude: Acai) {
        lock.withLock {
            acaiInstances.remove(amplitude)
            let identifier = ObjectIdentifier(amplitude)
    ...
    @objc static func didEndEditing(_ notification: NSNotification) {
        ...
        lock.withLock {
            for amplitude in acaiInstances.allObjects where amplitude.autocaptureManager.isEnabled(.elementInteractions) {
    ...
    fileprivate static func processFrustrationInteractionForView(_ view: UIView,
        ...
        lock.withLock {
            for amplitude in acaiInstances.allObjects {
                let identifier = ObjectIdentifier(amplitude)
...
extension UIApplication {
    @objc func amp_sendAction(_ action: Selector, to target: Any?, from sender: Any?, for event: UIEvent?) -> Bool {
        ...
        UIKitElementInteractions.lock.withLock {
            for amplitude in UIKitElementInteractions.acaiInstances.allObjects where amplitude.autocaptureManager.isEnabled(.elementInteractions) {
