...
public class ObjCPlugin: NSObject {
    ...
    internal let setup: ((ObjCAcai) -> Void)?
...
class ObjCPluginWrapper: Plugin, EventPlugin {
    weak var amplitude: ObjCAcai?
    let type: PluginType
    let wrapped: ObjCPlugin

    init(amplitude: ObjCAcai, wrapped: ObjCPlugin) {
        self.amplitude = amplitude
        self.type = wrapped.type
        self.wrapped = wrapped
    }

    func setup(amplitude: Acai) {
